...
int main() {
    ...
    auto mst_kruskal = [&](int removed = -1) {
        vi used;
        int total = 0;
    ...
    for (int mst_index : mst.second) {
        auto other_mst = mst_kruskal(mst_index);
        if (other_mst.second.size() == n - 1) {
            second = min(second, other_mst.first);
        }
    }
