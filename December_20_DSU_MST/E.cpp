...
int main() {
    ...
    const int ADD_EDGE = 1, CHECK_WEIGHT = 2;

    DSU dsu(n + 1);
    for (int i = 0; i < m; ++i) {
        int type = read_int();
        if (CHECK_WEIGHT == type) {
            int v = read_int();
            cout << dsu.get_size(v) << ENDL;
        } else {
            int from = read_int(), to = read_int(), weight = read_int();
            dsu.merge(from, to, weight, true);
        }
    }
