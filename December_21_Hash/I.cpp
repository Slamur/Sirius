...
struct HashRight {
    ...
    int solve(string & a, string & b) {
        ...
        auto a_hashes = calculate_hashes(a);
        auto b_hashes = calculate_hashes(b);

        int result = 0;

        for (int size = n; size > 0; --size) {
            int a_hash = get_hash(a_hashes, 0, size);
            int b_hash = get_hash(b_hashes, n - size, n);
