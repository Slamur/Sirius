...
struct HashRight {
    ...
    int find(string & s) {
        ...
        int s_hash = s_hashes[n];

        for (int i = 1; i <= n; ++i) {
            if (n % i != 0) continue;
            
            int p_hash = s_hashes[i];

            int full_p_hash = multiply<modulo>(p_hash,
                    subtract<modulo>(base_powers[n], 1)
            );

            int full_s_hash = multiply<modulo>(s_hash,
                    subtract<modulo>(base_powers[i], 1)
            );

            if (full_p_hash == full_s_hash) {
                return n / i;
            }
        }
