...
struct HashRight {
    ...
    vector<int> solve(const string & s_string) {
        size_t n = s_string.size();
        init_hash(n);

        vector<int> s(n);
        for (int i = 0; i < n; ++i) s[i] = s_string[i];

        auto reverse_s = reverse(s);

        auto s_hashes = calculate_hashes(s);
        auto reverse_hashes = calculate_hashes(reverse_s);
        ...
        for (int i = 1, j = n - 1; i <= n; ++i, --j) {
            ...
            while (left + 1 < right) {
                ...
                int i_hash = get_hash(s_hashes, 0, mid);
                int j_hash = (j_end > n ? -1 : get_hash(reverse_hashes, j, j_end));
